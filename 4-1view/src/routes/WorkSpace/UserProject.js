...
import {Tabs ,Table,Card,Modal, Button, Row,Col,Input,message,Divider, Icon ,Tooltip,Tag,Form} from 'antd';
...
        var changeDate = new Date(time);
    ...
    fetch = (params = {}) => {
            ...
            let data= {
                ...
                reportUser:current_user.RealName,
                ...params
            };
    ...
    fetchSearchForm=()=>{
      ...
      //表单条件提交
      onSearchValueChange=(changeVlaues,allValues)=>{
        let current_user=JSON.parse(getCurrentUser());
        var Project='';
        if(allValues.Project!==undefined) Project=allValues.Project.join(',');
        var params={
            Project:Project,
        };
        let data= {
    ...
    //             axios({
    //                     method:'get',
    //                     url: CONFIG.DeleteReports+'?ids='+selectKey.join(','),
    //                 }).then((response)=>{
    //                     console.log(response);
    ...
    render(){
        const { tags, inputVisible, inputValue ,projectData} = this.state;
        const rowSelection = {
       ...
       return (
        ...
        <div >
               <Modal
                ...
                <Card bordered={false}>
                    <SearchForm onSearchValueChange={this.onSearchValueChange}  projectData={projectData}></SearchForm>
                </Card>
...
class SearchForm extends React.Component{
    render(){
        const { form ,projectData,onSearchValueChange} = this.props;
        const { getFieldDecorator } = form;
       return (
            <Form layout="inline">
                <StandardFormRow title="项目" block style={{ paddingBottom: 2 }}>
                    <FormItem>
                        {getFieldDecorator('Project')(
                        <TagSelect expandable>
                        {
                            projectData.length>0? projectData.map((item,i)=>{
...
SearchForm=Form.create({ 
    onValuesChange({onSearchValueChange},changeVlaues,allValues){
        onSearchValueChange(changeVlaues,allValues)
    }
})(SearchForm);